...
function setToArray(mySet) {
  ...
  let arr = Array.from(setVal);
  let arrNew = arr.filter((value) => typeof value === "string");
  arrNew.sort();
  return arrNew;
}
...
function iterateSet(set) {
  ...
  let setKey = set.keys();
  for (let value of setKey) {
    console.log(value);
  }
  let setVal = set.values();
  for (let value of setVal) {
    console.log(value);
  }
  let setEntry = set.entries();
  for (let value of setEntry) {
    console.log(value);
  }
}
...
function sumNumbers(set) {
  ...
  // Повертаємо суму
  let sum = 0;
  set.forEach((elem) => (typeof elem === "number" ? (sum += elem) : false));
